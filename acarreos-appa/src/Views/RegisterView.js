...
function RegisterView() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length > 0) {
      ...
      setErrors({});
      try {
        const response = await fetch('http://34.176.155.184:5000/api/registerClient', {
          method: 'POST',
          headers: {
        ...
        login(data);  // Inicia sesión con los datos del usuario
        ...
        <div className="form-section text-sm">
          <form className="register-form" onSubmit={handleSubmit}>
            ...
            <p className='font-medium text-xs pb-4'>
              ¿Ya tienes una cuenta?, <a href="/login" className="register-link hover:underline">Inicia Sesión</a>
            </p>
            ...
            {apiError && <p className="error-message text-red-500">{apiError}</p>} {/* Muestra el error de la API */}
            
            <div style={{ textAlign: 'center' }}>
              <button type="submit" className="register-button mt-1 bg-[#00C853] h-10 p-2 rounded-md mb-10">
                <p className='text-sm text-black font-bold'>Registrarme</p>
              </button>
            </div>
          </form>
        </div>
